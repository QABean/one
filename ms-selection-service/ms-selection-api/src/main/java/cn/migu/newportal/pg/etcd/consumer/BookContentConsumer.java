package cn.migu.newportal.pg.etcd.consumer;


import com.google.protobuf.Descriptors;
import com.google.protobuf.Descriptors.ServiceDescriptor;

import cn.migu.millet.MilletContext;
import cn.migu.selection.proto.bookcontentservice._BookContentService.BookContentService;
import cn.migu.wheat.service.ServiceConsumer;

public class BookContentConsumer extends ServiceConsumer
{
    private static final String SERVICE = "ms.selection.bookContentService";

    private static Descriptors.ServiceDescriptor descriptor = BookContentService.getDescriptor();

    public BookContentConsumer(MilletContext context,String version) {
        super(context, SERVICE, version);
    }

    public void init() {
        super.initialize();
    }

    @Override
    protected ServiceDescriptor getServiceDescriptor()
    {
        return descriptor;
    }
}
